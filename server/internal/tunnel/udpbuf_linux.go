...
func trySetUDPBuffers(conn *net.UDPConn, size int) (actualRead, actualWrite int) {
	...
	if err != nil {
		// Fallback: use the standard API (will be capped by rmem_max).
	...
	// Fallback to regular setsockopt for whichever didn't succeed.
	if !rOK {
		_ = conn.SetReadBuffer(size)
	}
	if !wOK {
	...
	// Read back the actual buffer sizes the kernel applied.
	_ = raw.Control(func(fd uintptr) {
