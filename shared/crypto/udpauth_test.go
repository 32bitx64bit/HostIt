...
func TestBoundAndLegacyUDPRegistersAreDistinct(t *testing.T) {
	bound, _, _, _ := testBoundUDPRegister(t, "token", "agent-a")
	if _, _, _, ok := VerifyUDPRegister("token", bound, time.Now(), time.Minute); ok {
		t.Fatal("legacy parser accepted a bound register")
	}

	sessionID, _ := NewUDPSessionID()
	legacy, err := BuildUDPRegister("token", sessionID, "agent-a")
	if err != nil {
		t.Fatal(err)
	}
	if IsBoundUDPRegister(legacy) {
		t.Fatal("legacy register identified as bound")
	}
	if _, ok := ParseBoundUDPRegister("token", legacy, time.Now(), time.Minute); ok {
		t.Fatal("bound parser accepted a legacy register")
	}
}
