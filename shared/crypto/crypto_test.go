...
func BenchmarkCryptoConn_Write(b *testing.B) {
	...
	go func() {
		wc1, err1 = WrapTCP(c1, key, clientNonce, serverNonce, false)
		close(done2)
	}()
	wc2, err2 = WrapTCP(c2, key, clientNonce, serverNonce, true)
	<-done2
...
func TestCryptoConnHalfClose(t *testing.T) {
	...
	go func() {
		wServer, _ = WrapTCP(rawServer, key, clientNonce, serverNonce, false)
		close(done)
	}()
	wClient, err := WrapTCP(rawClient, key, clientNonce, serverNonce, true)
	if err != nil {
...
func TestWrapTCPLargeData(t *testing.T) {
	...
	go func() {
		...
		wc1, err := WrapTCP(c1, key, clientNonce, serverNonce, false)
		if err != nil {
			c1.Close()
			return
		}
	...
	wc2, err := WrapTCP(c2, key, clientNonce, serverNonce, true)
	if err != nil {
...
func TestCryptoConnReadBuffersPlaintextAcrossSmallReads(t *testing.T) {
	...
	client, err := WrapTCP(rawClient, key, clientNonce, serverNonce, true)
	if err != nil {
		t.Fatalf("WrapTCP client: %v", err)
	}
	server, err := WrapTCP(rawServer, key, clientNonce, serverNonce, false)
	if err != nil {
